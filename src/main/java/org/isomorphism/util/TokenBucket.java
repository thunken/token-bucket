...
public interface TokenBucket {
	...
	/**
	 * Returns the capacity of this token bucket. This is the maximum number of
	 * tokens that the bucket can hold at any one time.
	 *
	...
	/**
	 * Returns the current number of tokens in the bucket. If the bucket is
	...
	/**
	 * Returns the amount of time in the specified time unit until the next
	 * group of tokens can be added to the token bucket.
	 *
	 * @see org.isomorphism.util.TokenBucket.RefillStrategy#getDurationUntilNextRefill(java.util.concurrent.TimeUnit)
	 * @param unit
	 *            The time unit to express the return value in.
	 * @return The amount of time until the next group of tokens can be added to
	 *         the token bucket.
	 */
	...
	/**
	 * Refills the bucket with the specified number of tokens. If the bucket is
	 * currently full or near capacity then fewer than {@code numTokens} may be
	 * added.
	 *
	 * @param numTokens
	...
	static interface RefillStrategy {
		...
		/**
		 * Returns the amount of time in the specified time unit until the next
		 * group of tokens can be added to the token bucket. Please note,
		 * depending on the {@code SleepStrategy} used by the token bucket,
		 * tokens may not actually be added until much after the returned
		 * duration. If for some reason the implementation of
		 * {@code RefillStrategy} doesn't support knowing when the next batch of
		 * tokens will be added, then an {@code UnsupportedOperationException}
		 * may be thrown. Lastly, if the duration until the next time tokens
		 * will be added to the token bucket is less than a single unit of the
		 * passed in time unit then this method will return 0.
		 *
		 * @param unit
		 *            The time unit to express the return value in.
		 * @return The amount of time until the next group of tokens can be
		 *         added to the token bucket.
		 */
		long getDurationUntilNextRefill(TimeUnit unit) throws UnsupportedOperationException;
	}
	...
	static interface SleepStrategy {
		/**
		 * Sleep for a short period of time to allow other threads and system
